...
import { Card, SectionHeader, Field, NumberInput, TextInput, SelectInput, StatCard, Row, Badge, Toggle } from './ui';
import { ExportActions } from './ExportActions';

const CATEGORY_OPTIONS = TRENDYOL_CATEGORIES.map((c) => ({
  value: c.id,
  label: `${c.categoryMain} — ${c.categoryGroup}`,
}));

export function TrendyolPanel() {
  ...
  const resolved = useMemo(
    () => resolveTrendyolRate(categoryId, brand, isNewSeller, sellingPrice),
    [categoryId, brand, isNewSeller, sellingPrice]
  );
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
      <Card className="lg:col-span-2">
        <SectionHeader step="①" title="Trendyol — Category & Selling Price" />
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Category Group">
            <SelectInput value={categoryId} onValueChange={setCategoryId} options={CATEGORY_OPTIONS} />
          </Field>
          <Field label="Brand (for exceptions)" hint="e.g. Anker, Samsung, Sony — leave blank if none applies">
            <TextInput value={brand} onChange={(e) => setBrand(e.target.value)} placeholder="Anker" />
          </Field>
          <Field label="Selling Price on Trendyol (AED)" hint="What the customer pays — VAT inclusive">
            <NumberInput value={sellingPrice} onValueChange={setSellingPrice} />
          </Field>
          <Field label="Commission % (auto / override)">
            <NumberInput
              prefix={null}
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <Toggle checked={isNewSeller} onChange={setIsNewSeller} label="New seller rate" />
            <span className="text-xs text-muted">New Seller (20% off, first 60 days)</span>
          </div>
          <Badge tone="accent">{category ? `${category.categoryMain} → ${category.categoryGroup}` : 'No category'}</Badge>
          <Badge tone={resolved.source === 'brand-exception' ? 'profit' : 'neutral'}>
            {resolved.source === 'brand-exception'
              ? 'Brand exception applied'
              : resolved.source.startsWith('tier')
                ? `Tiered (${resolved.source === 'tier-below' ? '≤ AED 50' : '> AED 50'})`
                : 'Standard category rate'}
          </Badge>
          {feeOverride !== null && (
        ...
        <div className="mt-6 border-t border-border pt-5">
          <SectionHeader step="③" title="Trendyol Commission Breakdown" />
          ...
          <Row label={`Trendyol Commission (${formatPercent(breakdown.feePercent)})`} value={formatAED(breakdown.feeAmount)} />
          <Row label={`VAT on Commission (${rules.vatOnFees ? formatPercent(rules.vatPercent) : 'off'})`} value={formatAED(breakdown.vatOnFee)} muted />
          <Row label="Total Commission Deduction" value={formatAED(breakdown.totalDeduction)} strong />
          <Row label="Net Revenue After Commission" value={formatAED(breakdown.netRevenue)} strong />
        </div>
      </Card>
      ...
      <div className="flex flex-col gap-5">
        <Card>
          ...
          <div className="mt-4 border-t border-border pt-4">
            <ExportActions
              ...
              lines={[
                { label: 'Category', value: category ? `${category.categoryMain} — ${category.categoryGroup}` : '—' },
                { label: 'Seller Type', value: isNewSeller ? 'New Seller (Newcomer Rate)' : 'Standard' },
                ...
                { label: 'Trendyol Commission', value: `${formatAED(breakdown.feeAmount)} (${formatPercent(breakdown.feePercent)})` },
        ...
        <Card>
          ...
          <p className="mt-2 text-[11px] text-muted">Target margin = markup over cost, grossed up for VAT + commission.</p>
        </Card>
      </div>
    </div>
  );
